 ...
 * @param inode pointer to the ext2_inode_t structure to be checked.
 ...
 * @param inode pointer to the ext2_inode_t structure whose file type is to be determined.
 ...
 * @param volume pointer to the ext2_volume_t structure representing the filesystem volume.
 * @param inode pointer to the ext2_inode_t structure representing the inode whose block is being resolved.
...
kerr_t ext2_list_directory(ext2_volume_t *volume, uint32_t inode_number)
{
    ...
    if (kerr_failed(ext2_read_inode(volume, inode_number, &dir_inode)) || !inode_is_directory(&dir_inode))
    {
        return KERR_IO;
    }
