...
STAGE2_SECTORS := 16
KERNEL_SECTORS := 48
KERNEL_START_SEEK := $(shell echo $$(( 1 + $(STAGE2_SECTORS) )))
SECTOR_SIZE    := 512
...
$(STAGE2_BIN): $(STAGE2_SRC) $(STAGE2_C_SRC) $(STAGE2_OUTPUT_SRC) $(COMMON_OBJS) linker/stage2.ld
	mkdir -p $(dir $@)
	...
	$(LD) $(STAGE2_LDFLAGS) -o $(STAGE2_ELF) $(STAGE2_OBJ) $(STAGE2_C_OBJ) $(STAGE2_OUTPUT_OBJ) $(COMMON_OBJS)
...
$(KERNEL_ELF): $(KERNEL_MAIN_OBJ) $(KERNEL_C_OBJ) $(KERNEL_OUTPUT_OBJ) $(DRIVER_BLOCK_OBJ) $(DRIVER_ATA_OBJ) $(FS_EXT2_OBJ) $(KERNEL_FS_OBJ) $(COMMON_OBJS) linker/kernel.ld
	$(LD) $(LDFLAGS) -o $@ $(KERNEL_MAIN_OBJ) $(KERNEL_C_OBJ) $(KERNEL_OUTPUT_OBJ) $(DRIVER_BLOCK_OBJ) $(DRIVER_ATA_OBJ) $(FS_EXT2_OBJ) $(KERNEL_FS_OBJ) $(COMMON_OBJS)
...
$(DISK_IMAGE): $(STAGE1_BIN) $(STAGE2_PAD) $(KERNEL_PAD)
	...
	dd if=$(KERNEL_PAD) of=$@ bs=512 seek=$(KERNEL_START_SEEK) conv=notrunc status=none

run: check-tools $(DISK_IMAGE)
	@if [ -f $(EXT2_IMAGE) ]; then \
		$(QEMU) -drive if=floppy,format=raw,file=$(DISK_IMAGE) -drive if=ide,index=0,format=raw,file=$(EXT2_IMAGE); \
	else \
		echo "Note: $(EXT2_IMAGE) missing, running without ext2 side-disk"; \
...
run-ext2: check-tools $(DISK_IMAGE) ext2-image
	$(QEMU) -drive if=floppy,format=raw,file=$(DISK_IMAGE) -drive if=ide,index=0,format=raw,file=$(EXT2_IMAGE)
